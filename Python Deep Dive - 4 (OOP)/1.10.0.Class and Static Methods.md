...
As we saw, when we define a function inside a class, how it behaves (as a function or a method) depends on how the function is accessed: from the class, or the instance. (We'll cover how that works when we look at descriptors later in this course).
...
If we say `hello` from the class:
...
As you can see the instance `p` was passed as an argument to `hello`.

Sometimes, however, we define functions in a class that do not interact with the instance itself but may need something from the class. In those cases, we want the class to be passed to the function as an argument, whether it is called from the class or an instance of the class.
...
On the other hand, notice how the instance method when called from the instance and the class:
...
The static method can be called either from the class or the instance but is never bound:
...
We're going to create a `Timer` class that will allow us to get the current time (in both UTC and some time zones), as well as record start/stop times.
...
If you need to work with timezones, I recommend you use the `pyrz` 3rd party library. Here, I'll just use the standard library, which is not as easy to use as `pytz`.
...
class Timer:
    ...
    def set_tz(cls, offset, name):
        cls.tz = timezone(timedelta(hours=offset), name)s
```
...
Next, we want a function to return the current UTC. Obviously, this has nothing to do with either the class or the instance, so it is a prime candidate for a static method:
...
Next, we want a method that will return the current time based on the set time zone. The time zone is a class variable, so we'll need to access that, but we don't need any instance data, so this is a prime candidate for a class method:
