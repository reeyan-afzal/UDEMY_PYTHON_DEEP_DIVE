We can create our own exception types, by simply inheriting from `Exception`. (Usually, we want to inherit from `Exception`, not `BaseException` since `BaseException` includes exceptions such as `SystemExit`, `KeyboardInterrupt` , and a few others - our custom exceptions mostly do not fall under the same _base_ type of exceptions, but rather under `Exception`.
...
Let's say our library's name is _WebScraper_.
...
As you can see this was quite a lot of exception handling we had to do. And really, the HTTP status and message should remain consistent with any exception type.
...
We might have a custom exception just for that - remember what we discussed earlier, we might want our application to raise custom exceptions for everything, based on some application base exception our users could broadly trap.
