...
So this is where I am going to leave this implementation. In reality, there are a lot of other things we should consider such as additional validations, defining equality for two `Account` instances, and so on.

Also, one major piece we have not done is any kind of testing! We manually tested various pieces as we were coding, but there are multiple problems with this:
