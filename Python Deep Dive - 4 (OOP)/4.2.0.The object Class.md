...
You probably recognize that output! If we define our own class that does not **override** the `__repr__` or `__str__` methods, when we call those methods on instances of that class it will call the implementation in the `object` class:
...
This is called the `__str__` method in the `object` class (but it is an instance method, so it applies to our specific instance `p`).
...
And so we have the same functionality - our custom objects will compare equally only if they are the same object (id):
...
We can see what specific method is being called by looking at the id of the method in our object, and in the object class:
