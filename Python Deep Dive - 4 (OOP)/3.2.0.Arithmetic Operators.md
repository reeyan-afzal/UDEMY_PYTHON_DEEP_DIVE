Let's first look at some simple examples of using the straightforward `__add__`, `__sub__`, etc.
...
Now, let's support the addition and subtraction of vectors - they'll need to be of the same dimension, we should raise a `TypeError` exception (consistent with the exception Python raises if you try to add a string and an int for example).
...
Now, let's add support for multiplication by a scalar value - e.g. multiplying a vector by a real number (not another vector).
...
If you are rusty on this, just do a quick read of this: [https://en.wikipedia.org/wiki/Dot_product](https://en.wikipedia.org/wiki/Dot_product)
...
As you can see, the list `l` was mutated (memory address remained the same). This is not the same effect as:
...
But in-place does **not** _guarantee_ a mutation. For example, tuples are immutable objects:
...
As you can see we ended up with a new tuple. The same thing happens with strings, integers, floats, and so on, which are also immutable types.
...
- a list of children's `Person` objects

We want to make it such that we can add children simply by using in-place addition.
