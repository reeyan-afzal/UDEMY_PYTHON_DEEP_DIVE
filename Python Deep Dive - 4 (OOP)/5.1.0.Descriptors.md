...
There are two types of descriptors we need to discuss as I explain in the video lecture:
...
Suppose we want to create a class that allows us to select a random suit and a random card from that suit from a deck of cards (with replacement, i.e. the same card can be picked more than once).
...
Of course, we are not limited to just cards, we could use them in other classes too:
