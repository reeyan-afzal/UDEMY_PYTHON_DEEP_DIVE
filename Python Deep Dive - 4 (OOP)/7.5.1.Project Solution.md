...
To do that we cannot simply use the `str` or `repr` of the `__traceback__` property of the exception:
...
What's nice now, is that we could just print the traceback without logging the exception:
...
try:
    ...
    try:
        ...
        print(''.join(ex.traceback))s
```

Now we can define our exception subtypes, including the HTTP status for each:
...
class CannotStackCouponException(PricingException):
    message = 'Cannot stack checkout coupon codes.'
    http_status = HTTPStatus.BAD_REQUESTs
```
...
As you can see our traceback was empty above (the exception is present, but there is no call stack) - because we did not raise the exception!
