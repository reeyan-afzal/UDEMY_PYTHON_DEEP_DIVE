...
The `Student` instance no longer has an instance dictionary:
...
When we subclass a slot-less class, and define slots for the subclass, then we get a similar behavior to our first example - the subclass has both an instance dictionary and slots:
...
As we'll see later, behave essentially the same as properties - neither of them is actually stored in an instance dictionary - the additional effect of slots is that it (may) remove the need for an instance dictionary entirely.
...
We also do not have an instance dictionary:
...
So, as we can see neither the property `name` nor the slotted attribute `age` are stored in an instance dictionary.
...
Descriptors are objects that implement certain special functions (of course!) - just like iterators are objects that implement the special functions `__iter__` and `__next__`.
...
As we can see, we have an instance dictionary (that contains `age` since it was not defined in the `__slots__`, and `name` which was defined as a slot however, is not found in the instance dictionary.
