...
You should test this class by writing some unit tests!
...
The first thing is I'm going to add a "private" method that will indicate whether two objects are compatible. Maybe something like this:
...
But then I'm still left with which value I use, `.value` or the `int` itself. So, I'm going to make that part of the compatibility check.
...
Ok, so this is better, but there is still quite a bit of repetitive code for the addition, subtraction, multiplication, and power operations - the only thing that changes there is which particular arithmetic operator we are delegating to.
