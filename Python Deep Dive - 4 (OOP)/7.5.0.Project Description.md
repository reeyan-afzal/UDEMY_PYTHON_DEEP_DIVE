Suppose we have a Widget online sales application and we are writing the backend for it. We want a base `WidgetException` class that we will use as the base class for all the custom exceptions we raise from our Widget application.
...
- implement an HTTP status code associated with each exception type (keep it simple, use a 500 (server error) error for everything except invalid coupon code, and cannot stack coupons, these can be 400 (bad request)
...
[https://docs.python.org/3/library/traceback.html#tracebackexception-objects](https://docs.python.org/3/library/traceback.html#tracebackexception-objects)
