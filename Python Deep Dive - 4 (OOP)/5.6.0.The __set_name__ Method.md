...
Now watch what happens when we get the property from the instances:
...
That can be handy for messages that can reference the property name, or even store values in the instance dictionary (assuming we can):
