...
Now we can create multiple instances of that type:
...
We did this: `if getattr(cls, 'existing_instance')`
...
So, instead, we are going to store the class, and the instance of that class, in a dictionary **in the metaclass** itself, and use that dictionary to look up the existing instance (if any) for a specific class.
