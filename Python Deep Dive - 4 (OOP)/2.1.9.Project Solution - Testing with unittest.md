...
Let's see how some simple tests are set up and executed:
...
Even if the test fails, the teardown method will still run:
...
Another thing to watch out for is that there is no guarantee of the order in which the unit tests are run. The best practice is that unit tests should be independent of each other.
