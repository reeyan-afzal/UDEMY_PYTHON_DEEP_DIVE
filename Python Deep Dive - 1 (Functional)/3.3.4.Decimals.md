...
Another mechanism or another way to represent floating points numbers. The ```decimal``` module for more details you may read **PEP 327**.
...
Finite number of significant digits -> rational number, if you're still confused then do check the previous notes. But the main question here is that why not just use the ```Fraction``` class?
...
Finance, banking, and any other field where exact finite representations are highly desirable. Let's say we are adding up all the financial transactions that took place over a certain time period:
...
'''creates a new context, copied from ctx or from default if ctx not specified returns a context manager (use a with statement)'''
```
