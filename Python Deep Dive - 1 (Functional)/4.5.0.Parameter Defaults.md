...
The function is **executed** and by the time this happens, the default value for ```a``` has **already** been evaluated and assigne - it is **not re-evaluated** when the function is called
...
As you can see for both logs the **dt** was the same because during function creation it has been defined and doesn't update during function call.
#### Solution Pattern
