...
Some languages (such as Java, C, ...) provide multiple distinct integer data types that used a fixed number of bits:
...
Python does not work this way because in python **int** object used a variable number of bits. Can use **4** bytes (32 bits), **8** bytes (64 bits), **12** bytes (96 bits), etc. **Seamless to us**

Since **int**s are actually objects, there is a further fixed overhead per integer. Theoretically limited only by the  amount of memory available. Of course, larger numbers will use more memory and standard operators such as ```+, *``` etc. will run slower as numbers get larger.
