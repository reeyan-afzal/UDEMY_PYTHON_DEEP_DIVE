...
You can see a PEP that discussed adding a `switch` statement to Python, proposed by Guido, but ultimately rejected (by Guido as well): [https://www.python.org/dev/peps/pep-3103/](https://www.python.org/dev/peps/pep-3103/)
...
But the downside of this approach compared to `if...elif...else` is that the dictionary values are relatively simple and cannot contain nested if statements or anything else. In the case of `if...elif...else` your code blocks for each of this statement can contain as many lines of code as you want.
