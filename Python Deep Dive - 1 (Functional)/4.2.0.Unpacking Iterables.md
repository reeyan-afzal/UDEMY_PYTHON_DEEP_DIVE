...
Packed values refers to values that are **bundled** together in some way. Tuples and Lists are obvious:
...
```a, b, c = [1, 2, 3]``` **3** elements in ```[1, 2, 3]``` -> need **3** variables to unpack. This is actually a tuple of **3** variables: ```a```, ```b``` and ```c```
...
```a, b, c = 10, 20, 'hello'``` this is actually a tuple containing 3 values 
