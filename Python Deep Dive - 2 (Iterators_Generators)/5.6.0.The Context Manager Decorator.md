...
Next, let's re-create the context manager wrapper we did in the last video, but this time, I'm going to pass it a generator object, instead of a generator function. But the same idea:
...
We can read back from the file too:
...
Fortunately the standard library already has this implemented for us - that was one of the critical goals of Python's context managers - the ability to create context managers using generator functions (see PEP 343).
